...
const UserProfileAvatarEdit = ({close}) => {
    ...
    try {
        ...
        const response = await Axios({
            ...SummaryApi.uploadAvatar, // { method: 'PUT', url: '/api/user/upload-avatar' }
            data: formData
        });

        const avatarUrl = response?.data?.data?.avatar || ""; // safe access

        if (avatarUrl) {
            dispatch(updatedAvatar(avatarUrl)); // Redux update
        } else {
            console.warn("Avatar URL missing in response:", response);
        }
